...
class indexer extends \core_search\base {
    ...
    public function get_document($record) {
        ...
        $userdata = $this->get_user_names_data($USER->id);

        // Prepare associative array with data from DB.
        $doc = \core_search\document_factory::instance($record->id, 'core_mocksearch');
        $doc->set('title', $record->capability . ' roleid ' . $record->roleid);
        $doc->set('content', $record->capability . ' roleid ' . $record->roleid . ' message');
        $doc->set('contentformat', FORMAT_MOODLE);
        $doc->set('userfullname', fullname($userdata));
        $doc->set('contextid', $record->contextid);
        $doc->set('type', \core_search\manager::TYPE_TEXT);
        $doc->set('courseid', SITEID);
        $doc->set('userid', $USER->id);
        $doc->set('created', time() - 10);
        $doc->set('modified', time());
        $doc->set('name', 'the name');
        $doc->set('intro', 'the intro');
        $doc->set('contentformat', FORMAT_PLAIN);
