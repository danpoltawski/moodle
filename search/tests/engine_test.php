...
class search_engine_testcase extends advanced_testcase {
    ...
    public function test_engine_caches() {
        ...
        $this->assertFalse($engine->get_search_component('plugintype_unexisting'));
        $this->assertFalse($engine->get_search_component('mod_unexisting'));
        $this->assertFalse($engine->get_search_component('mod_unexisting'));

        $this->assertInstanceOf('\\mod_forum\\search\\indexer', $engine->get_search_component('mod_forum'));
        $dbreads = $DB->perf_get_reads();
        $this->assertInstanceOf('\\mod_forum\\search\\indexer', $engine->get_search_component('mod_forum'));
        $this->assertEquals($dbreads, $DB->perf_get_reads());
