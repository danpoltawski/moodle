...
/**
 * Various PHPUnit class and functions
 *
 * @package    core_core
 * @category   phpunit
...
require_once 'PHPUnit/Autoload.php'; // necessary when loaded from Moodle CLI script
...
class phpunit_util {
    ...
    /**
     * Resets contents of all database tables to initial values, reset caches, etc.
     *
    ...
    /**
     * Return original state of global variable.
     * @static
     * @param string $name
     * @return mixed
     */
    ...
    public static function install_site() {
        ...
        // hash all plugin versions - helps with very fast detection of db changes
        $hash = phpunit_util::get_version_hash();
        @unlink("$CFG->dataroot/phpunit/versionshash.txt");
    ...
    <testsuites>
        <testsuite name="@component@">
            <directory suffix="Test.php">@dir@</directory>
        </testsuite>
    </testsuites>';
 ...
 *
 * Note: this is supposed to work only for very simple tests only.
 *
 * @deprecated
 * @package    core_phpunit
 * @author     Petr Skoda
    ...
    public function runBare() {
        ...
        if ($oldcfg) {
            foreach($oldcfg as $k=>$v) {
                $CFG->$k = $v;
                error_log('warning: unexpected removeal of $CFG->'.$k.' in testcase: '.get_class($this).'->'.$this->getName(true));
            }
        }
